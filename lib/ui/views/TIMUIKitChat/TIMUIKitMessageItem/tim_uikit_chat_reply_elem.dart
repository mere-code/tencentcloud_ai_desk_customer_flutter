...
class _TIMUIKitReplyElemState extends TIMUIKitState<TIMUIKitReplyElem> {
  ...
  Widget _defaultRawMessageText(String text, TUITheme? theme) {
    ...
    return Text(text,
        ...
        style: TextStyle(
            fontSize: 12,
            color: isFromSelf ? Colors.white : theme?.weakTextColor,
  ...
  Widget tuiBuild(BuildContext context, TUIKitBuildValue value) {
    ...
    return Container(
      padding: widget.textPadding ?? EdgeInsets.all(isDesktopScreen ? 12 : 11),
      decoration: BoxDecoration(
        color: (isFromSelf && !isShowJumpState) ? null : backgroundColor,
        gradient: (isFromSelf && !isShowJumpState) ? LinearGradient(
